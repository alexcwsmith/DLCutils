...
sampleName = 'F2-RT_Cat_Phase1'
suffix = 'DLC_resnet50_npwJul21shuffle1_50000_filtered.csv'
fileName = strcat(sampleName,suffix)
filePath = '/d1/studies/DeepLabCut/DLCutils/MotionMapper/';
...
nose_y = dataArray{:, 3};
nose_lh = dataArray{:, 4};
tailbase_x = dataArray{:, 5};
tailbase_y = dataArray{:, 6};
tailbase_lh = dataArray{:, 7};
spine1_x = dataArray{:, 8};
spine1_y = dataArray{:, 9};
spine1_lh = dataArray{:, 10};
spine2_x = dataArray{:, 11};
spine2_y = dataArray{:, 12};
spine2_lh = dataArray{:, 13};
spine3_x = dataArray{:, 14};
spine3_y = dataArray{:, 15};
spine3_lh = dataArray{:, 16};
forepaw_r_x = dataArray{:, 17};
forepaw_r_y = dataArray{:, 18};
forepaw_r_lh = dataArray{:, 19};
forepaw_l_x = dataArray{:, 20};
forepaw_l_y = dataArray{:, 21};
forepaw_l_lh = dataArray{:, 22};
hindpaw_r_x = dataArray{:, 23};
hindpaw_r_y = dataArray{:, 24};
hindpaw_r_lh = dataArray{:, 25};
hindpaw_l_x = dataArray{:, 26};
hindpaw_l_y = dataArray{:, 27};
hindpaw_l_lh = dataArray{:, 28};

% Compute Projections Matrix: (put in your body parts list!)
AllBodyparts = horzcat(nose_x, nose_y, tailbase_x, tailbase_y, spine1_x, spine1_y, spine2_x, spine2_y, spine3_x, spine3_y, forepaw_r_x, forepaw_r_y, forepaw_l_x, forepaw_l_y, hindpaw_r_x, hindpaw_r_y, hindpaw_l_x, hindpaw_l_y);
% align to a set bodypart, here "tailbase":
AllBodypartsMinusOne = horzcat(nose_x-tailbase_x, nose_y-tailbase_y, tailbase_x-tailbase_x, tailbase_y-tailbase_y, spine1_x-tailbase_x, spine1_y-tailbase_y, spine2_x-tailbase_x, spine2_y-tailbase_y, spine3_x-tailbase_x, spine3_y-tailbase_y, forepaw_r_x-tailbase_x, forepaw_r_y-tailbase_y, forepaw_l_x-tailbase_x, forepaw_l_y-tailbase_y, hindpaw_r_x-tailbase_x, hindpaw_r_y-tailbase_y, hindpaw_l_x-tailbase_x, hindpaw_l_y-tailbase_y);
...
parameters.numProcessors = 16; %8; %4
parameters.displayAlignmentExample = false;
parameters.samplingFreq = 30; %30
...
savefig(strcat(fileName,'_Embeddings.fig'))
...
figure
set(gcf, 'color','w')
videoSuffixFull = '_Video_Full_pause_15FPS.avi';
videoFull = VideoWriter(strcat(sampleName, videoSuffixFull));
videoFull.FrameRate=15;
for j=1:2:N %was j=1:4:N
    %middle panel:
    subplot(3,3,5) %[2 5 8]
    N = length(projections(:,1));
    for i=0:numRegions
   ...
   % vidSuffixEtho = '_Video_ethogram.avi'
   % videoEtho = VideoWriter(strcat(sampleName, vidSuffixEtho))
   % videoEtho.FrameRate = 30
    subplot(3,3,4)
    imagesc((1:N)./parameters.samplingFreq,0:numRegions,ethogram')
    colormap(cc)
    caxis([0 numRegions]);
...
;
    ...
    frame = getframe(gcf);
    open(videoFull);
    writeVideo(videoFull, frame); 

end
close(videoFull);

savefig(strcat(fileName,'_SummaryFig.pdf'), '-dpdf')
